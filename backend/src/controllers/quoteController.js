...
const sendQuote = async (req, res) => {
  try {
    ...
    if (!worker || !worker.isActive || worker.verification.status !== 'verified') {
      return res.status(403).json({ message: 'Worker is not eligible to quote' });
    }
...
const rejectQuote = async (req, res) => {
  try {
    ...
    booking.rejectionReason = typeof req.body?.reason === 'string'
      ? req.body.reason.trim().slice(0, 300)
      : 'Customer rejected the worker quote';
    booking.quote.customerRespondedAt = new Date();
    await booking.save();
