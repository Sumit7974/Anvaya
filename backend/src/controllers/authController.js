...
const registerUser = (role) => async (req, res) => {
  try {
    ...
    if (role === 'admin') {
      ...
      if (!bootstrapKey || req.get('x-admin-bootstrap-key') !== bootstrapKey) return res.status(403).json({ message: 'Admin registration is restricted. Use the provisioned admin account.' });
    }
    ...
    if (!normalizedName || !normalizedEmail || !EMAIL_RE.test(normalizedEmail) || !password || (role !== 'admin' && !normalizedPhone)) return res.status(400).json({ message: 'Please provide valid required fields' });
    if (typeof password !== 'string' || password.length < 8 || password.length > 128) return res.status(400).json({ message: 'Password must be 8 to 128 characters long' });
...
const loginUser = (role) => async (req, res) => {
  try {
    ...
    const normalizedEmail = typeof email === 'string' ? email.trim().toLowerCase() : '';
    if (!normalizedEmail || !EMAIL_RE.test(normalizedEmail) || typeof password !== 'string') return res.status(400).json({ message: 'Email and password are required' });
