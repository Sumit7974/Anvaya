...
const labels = { requested: 'Request Sent', accepted: 'Accepted', 'quote-pending': 'Price Proposed', 'in-progress': 'Work Started', 'completion-pending': 'Verify Work', completed: 'Completed' };
...
function WorkerProcess({ worker, bookingId, onBack, onProceedToPayment, onComplaint, onRetryWorker }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const found = (data.bookings || []).find((item) => item._id === bookingId);
...
{booking?.status === 'completion-pending' && <div className="mt-6 rounded-3xl border-2 border-purple-200 bg-purple-50 p-6"><h2 className="text-xl font-bold text-purple-900">🛡️ Customer satisfaction check</h2><p className="mt-2 text-sm leading-6 text-purple-800">Do not confirm just because the worker says it is done. Inspect and test the work first.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><button type="button" disabled={busy} onClick={() => void completion('dispute-completion')} className="rounded-xl border border-red-200 bg-white px-4 py-3 font-bold text-red-600">Report Problem</button><button type="button" disabled={busy} onClick={() => void completion('confirm-completion')} className="rounded-xl bg-emerald-600 px-4 py-3 font-bold text-white">✓ Work Satisfactory</button></div><button type="button" onClick={onComplaint} className="mt-3 w-full rounded-xl border border-red-200 bg-white px-4 py-3 text-sm font-bold text-red-600">Submit a formal complaint</button><p className="mt-3 text-center text-xs text-purple-700">Payment stays locked until you confirm.</p></div>}
{booking?.status === 'disputed' && <div className="mt-6 rounded-2xl bg-red-50 p-5 text-red-700"><b>Payment locked</b><p className="text-sm">You reported a problem. Submit a formal complaint if the issue needs review.</p><button type="button" onClick={onComplaint} className="mt-3 rounded-xl bg-red-600 px-4 py-2 font-bold text-white">Open Complaint</button></div>}
{booking?.status === 'completed' && <div className="mt-6 rounded-2xl bg-emerald-50 p-5"><b>✓ Work confirmed</b><p className="mt-1 text-sm">You confirmed the service is satisfactory.</p><button type="button" onClick={onProceedToPayment} className="mt-4 rounded-xl bg-emerald-600 px-5 py-3 font-bold text-white">Proceed to Payment →</button></div>}
