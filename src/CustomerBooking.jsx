...
function CustomerBooking({ onBack, onSelectWorker }) {
  ...
  const loadNearbyWorkers = async (coords) => {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async position => {
        ...
        try {
          await loadNearbyWorkers(coords);
        } catch (e) {
          setError(e.message || 'Unable to find nearby workers.');
          setWorkers([]);
        } finally {
          setLoading(false);
        }
      ...
      geolocationError => {
        ...
        if (geolocationError.code === geolocationError.PERMISSION_DENIED) {
          ...
          setError('Location took too long to load. Please try again.');
        } else {
          setError('We could not detect your location. Please try again.');
        }
      },
  ...
  const preparedWorkers = useMemo(() => {
    const q = search.trim().toLowerCase();
    return workers
      .map(worker => {
        const coordinates = worker.location?.coordinates;
        const hasLocation = Array.isArray(coordinates) && coordinates.length === 2 && customerLocation;
      ...
      .filter(worker => {
        ...
        const serviceMatches = service === 'All' || skills.some(skill => String(skill).toLowerCase() === service.toLowerCase());
        const searchMatches = !q || String(worker.name || '').toLowerCase().includes(q) || skills.some(skill => String(skill).toLowerCase().includes(q));
        return serviceMatches && searchMatches;
      })
      .sort((a, b) => (a.distance ?? Number.POSITIVE_INFINITY) - (b.distance ?? Number.POSITIVE_INFINITY));
  }, [workers, search, service, customerLocation]);

  const initialised = useMemo(() => ({ value: true }), []);
  if (initialised.value) {
    ...
    try {
      ...
      if (savedLocation?.latitude && savedLocation?.longitude) setCustomerLocation(savedLocation);
    } catch {
      // Ignore malformed local storage and continue normally.
    }
    window.setTimeout(() => detectLocation(), 0);
  }
  ...
  const analyze = async () => {
    ...
    try {
      setError('');
      const data = await apiRequest('/api/services/analyze', {
        method: 'POST',
      ...
      setError(e.message || 'Could not understand the request.');
    }
  };

  const submit = async () => {
    if (!selected || note.trim().length < 10) {
      setError('Please describe exactly what you need (at least 10 characters).');
      return;
    }
    if (!customerLocation) {
      setError('Your location is required so the booking can be tied to the correct service location.');
      return;
    }
    ...
    try {
      setSubmitting(true);
      setError('');
      const coords = [customerLocation.longitude, customerLocation.latitude];
      const data = await apiRequest('/api/bookings', {
        method: 'POST',
        token: getStoredToken(),
        body: {
          problemDescription: note.trim(),
          serviceTag: analysis?.service || selected.skills?.[0] || 'general service',
      ...
      setError(e.message || 'Unable to send booking request.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return <main className="min-h-screen bg-[#FFF8F3] text-slate-800">
    <header className="border-b border-amber-100 bg-white">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
        <div className="flex items-center gap-4">
          <img src="/anvaya-logo.png" alt="Anvaya" className="h-12" />
          <div className="border-l border-slate-200 pl-4">
            <p className="text-xs font-bold uppercase tracking-widest text-amber-700">Customer</p>
            <p className="text-sm font-semibold">Find & hire a trusted worker</p>
          </div>
        </div>
        <button type="button" onClick={onBack} className="rounded-xl border border-amber-200 bg-white px-4 py-2 font-semibold">← Back</button>
      </div>
    </header>
    ...
    <section className="bg-[#FFF1E6]">
      <div className="mx-auto max-w-7xl px-5 py-10">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-amber-700">Smart service request</p>
            <h1 className="mt-2 text-3xl font-bold">Trusted workers near you</h1>
            <p className="mt-2 max-w-2xl text-slate-600">We use your device location to show active, verified workers within {RADIUS_KM} km, sorted by distance.</p>
          </div>
          <div className={`rounded-2xl border px-4 py-3 text-sm font-bold ${locationStatus === 'ready' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-white text-amber-700'}`}>
            {locationStatus === 'ready' ? '📍 Location detected' : locationStatus === 'loading' ? '📍 Detecting location…' : '📍 Location needed'}
          </div>
        </div>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search electrician, plumber, carpenter..." className="mt-6 w-full max-w-2xl rounded-2xl border border-amber-100 bg-white px-5 py-4 outline-none" />
      </div>
    </section>

    <section className="mx-auto max-w-7xl px-5 py-7">
      {note.trim() && <div className="mb-6 rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-sm text-emerald-900"><p className="font-bold">✓ Your request is saved</p><p className="mt-1 leading-6">The worker will receive this exact note:</p><p className="mt-3 rounded-xl bg-white px-4 py-3 font-medium leading-6 text-slate-700">“{note}”</p></div>}
      {locationStatus !== 'ready' && <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-amber-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-slate-900">We need your location</p><p className="mt-1 text-sm leading-6 text-slate-500">Your location stays on this device and is used to find nearby workers and set the service location.</p></div><button type="button" onClick={detectLocation} className="rounded-xl bg-amber-600 px-5 py-3 font-bold text-white">Allow location</button></div>}
    ...
    <section className="mx-auto max-w-7xl px-5 pb-12">
      <div className="mb-5 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-bold">Available nearby workers</h2><p className="mt-1 text-sm text-slate-500">Only active, verified and available professionals are shown.</p></div>{locationStatus === 'ready' && <span className="text-sm font-bold text-emerald-700">{preparedWorkers.length} found</span>}</div>
      {loading ? <div className="rounded-2xl bg-white p-10 text-center text-slate-500">Finding verified workers near you…</div> : locationStatus !== 'ready' ? <div className="rounded-2xl bg-white p-10 text-center"><p className="font-bold text-slate-800">Location access is needed</p><p className="mt-2 text-sm text-slate-500">Allow location access to see nearby workers instead of workers from far away.</p></div> : preparedWorkers.length === 0 ? <div className="rounded-2xl bg-white p-10 text-center text-slate-500">No matching verified workers are available within {RADIUS_KM} km right now.</div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{preparedWorkers.map(w => <article key={w._id} className="rounded-3xl border border-amber-100 bg-white p-5 shadow-sm"><div className="flex justify-between"><span className="text-3xl">{icons[String(w.skills?.[0] || '').toLowerCase()] || '👷'}</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">● Available</span></div><h3 className="mt-5 text-xl font-bold">{w.name}</h3><p className="font-semibold text-amber-700">{title(w.skills?.[0])}</p><p className="mt-3 text-sm">⭐ {Number(w.rating?.average || 0).toFixed(1)} · {w.rating?.count || 0} reviews</p><p className="mt-2 text-sm font-semibold text-slate-600">📍 {w.distance == null ? 'Distance unavailable' : `${w.distance.toFixed(1)} km away`}</p><button type="button" onClick={() => choose(w)} className="mt-5 w-full rounded-xl bg-amber-600 px-4 py-3 font-bold text-white">Hire this worker →</button></article>)}</div>}
    </section>
