...
function CustomerBooking({ onBack, onSelectWorker }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    loadNearbyWorkers(customerLocation, service).catch(e => setError(e.message || 'Unable to refresh nearby workers.')).finally(() => setLoading(false));
  }, [service]);
  ...
  const filtered = useMemo(() => {
    ...
    return workers.map(worker => {
      ...
      return !q || String(worker.name || '').toLowerCase().includes(q) || skills.some(s => String(s).toLowerCase().includes(q));
    }).sort((a, b) => (a.distance ?? Number.POSITIVE_INFINITY) - (b.distance ?? Number.POSITIVE_INFINITY));
  }, [workers, search, customerLocation]);
