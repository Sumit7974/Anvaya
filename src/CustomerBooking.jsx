...
const VALID_SERVICE_TAGS = new Set(SERVICES.slice(1).map(item => item.toLowerCase()));
...
function CustomerBooking({ onBack, onSelectWorker }) {
  ...
  const initialLocation = Number.isFinite(Number(savedLocation?.latitude)) && Number.isFinite(Number(savedLocation?.longitude)) ? { latitude: Number(savedLocation.latitude), longitude: Number(savedLocation.longitude), label: savedLocation.label || 'Saved service location', precision: savedLocation.precision || 'exact' } : null;
  ...
  const requestCurrentLocation = () => {
    ...
    }, err => { setLocationRequestPending(false); setError(err.code === 1 ? 'Location permission was denied. Search the service address instead.' : 'Could not get your current location.'); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 120000 });
  };
  ...
  const submit = async () => {
    ...
    const analyzedService = String(analysis?.service || analysis?.serviceTag || '').trim().toLowerCase();
    const selectedService = String(selectedWorker.displayedSkill || '').trim().toLowerCase();
    const fallbackService = detectService(note);
    const serviceTag = VALID_SERVICE_TAGS.has(analyzedService) ? analyzedService : (VALID_SERVICE_TAGS.has(selectedService) ? selectedService : fallbackService);
    try {
      setBookingLoading(true); setError('');
      const data = await apiRequest('/api/bookings', { method: 'POST', token: getStoredToken(), body: { problemDescription: note.trim(), serviceTag: serviceTag || undefined, location: { type: 'Point', coordinates: [serviceLocation.longitude, serviceLocation.latitude] }, workerId: selectedWorker._id } });
      if (!data?.booking?._id) throw new Error('Booking was not created.');
      localStorage.removeItem('anvaya_service_request');
      setSelectedWorker(null);
      onSelectWorker?.({ ...selectedWorker, displayedSkill: serviceTag || selectedWorker.displayedSkill }, data.booking._id);
