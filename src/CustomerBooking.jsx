...
function CustomerBooking({ onBack, onSelectWorker }) {
  ...
  const initialLocation = Number.isFinite(Number(savedLocation?.latitude)) && Number.isFinite(Number(savedLocation?.longitude))
    ? { latitude: Number(savedLocation.latitude), longitude: Number(savedLocation.longitude), label: savedLocation.label || 'Saved location', precision: savedLocation.precision || 'exact' }
    : null;
  ...
  useEffect(() => {
    ...
    if (!navigator.geolocation) {
      ...
      setError('Your browser does not support location. Choose the service location manually.');
      return undefined;
    }
    let alive = true;
    navigator.geolocation.getCurrentPosition(
      ...
      locationError => {
        if (!alive) return;
        setLocationLoading(false);
        setError(locationError.code === 1 ? 'Location access was denied. You can still choose the service location manually.' : 'We could not detect your location. You can choose the service location manually.');
      },
  ...
  const workersWithDistance = useMemo(() => {
    ...
    return workers.map(worker => {
      ...
      const skills = Array.isArray(worker.skills) ? worker.skills.map(skill => String(skill).toLowerCase()) : [];
      const skillMatch = service === 'All' || skills.includes(service.toLowerCase());
      const searchMatch = !q || String(worker.name || '').toLowerCase().includes(q) || skills.some(skill => skill.includes(q));
      return skillMatch && searchMatch;
    }).sort((a, b) => (a.distance ?? Infinity) - (b.distance ?? Infinity));
  }, [workers, search, service, serviceLocation]);

  const saveLocation = next => {
    const normalized = { latitude: Number(next.latitude), longitude: Number(next.longitude), label: next.label || 'Selected service location', precision: next.precision || 'exact' };
    setServiceLocation(normalized); setLocationPickerOpen(false); setError('');
    localStorage.setItem('anvaya_customer_location', JSON.stringify(normalized));
  };
  ...
  const useCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      position => saveLocation({ latitude: position.coords.latitude, longitude: position.coords.longitude, label: 'Current location', precision: 'exact' }),
      locationError => { setLocationLoading(false); setError(locationError.code === 1 ? 'Location permission was denied.' : 'Could not get your current location.'); },
      { enableHighAccuracy: true, timeout: 10000, maximumAge: 120000 }
    );
  };
  ...
  const searchAddress = async () => {
    ...
    if (query.length < 3) { setError('Enter at least a town, village, landmark, or address.'); return; }
    ...
    try {
      ...
      if (!results.length) setError('No matching location found. Try adding the area, district, PIN code, or a nearby landmark.');
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await apiRequest('/api/bookings', {
        ...
        body: { problemDescription: note.trim(), serviceTag: analysis?.service || selectedWorker.displayedSkill || selectedWorker.skills?.[0] || 'general service', location: { type: 'Point', coordinates: [serviceLocation.longitude, serviceLocation.latitude] }, workerId: selectedWorker._id }
      });
  ...
  const locationPrecisionMessage = serviceLocation?.precision === 'area'
    ? 'This is an area-level location. For accurate doorstep distance, search the full service address or use GPS at the service address.'
  ...
  return <main className="min-h-screen bg-[#FFF8F3] text-slate-800">
    ...
    <section className="bg-[#FFF1E6]"><div className="mx-auto max-w-7xl px-5 py-9"><div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-xs font-bold uppercase tracking-widest text-amber-700">Smart service request</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Find a trusted worker near the job</h1><p className="mt-2 max-w-3xl text-slate-600">Tell us where the work needs to happen. We then show active, verified workers within {RADIUS_KM} km of that location.</p></div><button type="button" onClick={() => setLocationPickerOpen(true)} className="min-w-[270px] rounded-2xl bg-white p-5 text-left shadow-sm ring-1 ring-amber-200"><span className="block text-xs font-bold uppercase tracking-wider text-amber-700">Where is the work?</span><span className="mt-1 block font-bold text-slate-900">📍 {serviceLocation?.label || (locationLoading ? 'Detecting your location…' : 'Choose a location')}</span><span className="mt-2 block text-sm font-semibold text-amber-700">Change location →</span></button></div><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search by worker name or skill..." className="mt-6 w-full max-w-2xl rounded-2xl border border-amber-100 bg-white px-5 py-4 outline-none focus:ring-4 focus:ring-amber-100"/></div></section>

    <section className="mx-auto max-w-7xl px-5 py-7">
      {note.trim() && <div className="mb-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-5"><p className="font-bold text-emerald-900">✓ Your request is ready</p><p className="mt-1 text-sm text-emerald-800">The worker will receive exactly this note:</p><p className="mt-3 rounded-xl bg-white px-4 py-3 leading-6 text-slate-700">“{note}”</p></div>}
      {serviceLocation && <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Service location</p><p className="mt-1 font-bold text-slate-900">📍 {serviceLocation.label}</p><p className="mt-1 text-sm text-slate-500">{locationPrecisionMessage}</p></div><button type="button" onClick={() => setLocationPickerOpen(true)} className="rounded-xl border border-amber-200 px-4 py-2.5 text-sm font-bold text-amber-700">Change</button></div></div>}
    ...
    <section className="mx-auto max-w-7xl px-5 pb-12"><div className="mb-5 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-bold">Nearby workers</h2><p className="mt-1 text-sm text-slate-500">Verified, active and available professionals are shown first.</p></div>{serviceLocation && <span className="text-sm font-bold text-emerald-700">{workersWithDistance.length} found</span>}</div>{locationLoading && !serviceLocation ? <div className="rounded-2xl bg-white p-10 text-center text-slate-500">Detecting your location…</div> : !serviceLocation ? <div className="rounded-2xl bg-white p-10 text-center"><p className="font-bold">Choose the service location</p><p className="mt-2 text-sm text-slate-500">Use your current location or search for the place where the work needs to happen.</p><button type="button" onClick={() => setLocationPickerOpen(true)} className="mt-4 rounded-xl bg-amber-600 px-5 py-3 font-bold text-white">Choose location</button></div> : workersWithDistance.length === 0 ? <div className="rounded-2xl bg-white p-10 text-center text-slate-500">No matching verified workers are available within {RADIUS_KM} km of this service location.</div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{workersWithDistance.map(worker => <article key={worker._id} className="rounded-3xl border border-amber-100 bg-white p-5 shadow-sm"><div className="flex justify-between"><span className="text-3xl">{ICONS[worker.displayedSkill] || '👷'}</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">● Available</span></div><h3 className="mt-5 text-xl font-bold">{worker.name}</h3><p className="font-semibold text-amber-700">{title(worker.displayedSkill)}</p><p className="mt-3 text-sm">⭐ {Number(worker.rating?.average || 0).toFixed(1)} · {worker.rating?.count || 0} reviews</p><p className="mt-2 text-sm font-semibold text-slate-600">📍 {worker.distance == null ? 'Distance unavailable' : `${worker.distance.toFixed(1)} km from service location`}</p><button type="button" onClick={() => selectWorker(worker)} className="mt-5 w-full rounded-xl bg-amber-600 px-4 py-3 font-bold text-white">Hire this worker →</button></article>)}</div>}</section>

    {locationPickerOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4"><div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-amber-700">Service location</p><h2 className="mt-1 text-2xl font-bold">Where should the worker go?</h2><p className="mt-2 text-sm leading-6 text-slate-500">This can be your home, your parents’ home, a relative’s house, or another place that needs service.</p></div><button type="button" onClick={() => setLocationPickerOpen(false)} className="rounded-lg px-3 py-2 text-xl text-slate-500">✕</button></div><button type="button" onClick={useCurrentLocation} className="mt-6 flex w-full items-center justify-between rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-left"><span><b className="block text-emerald-800">📍 Use my current location</b><span className="text-sm text-emerald-700">Best when you are physically at the service address.</span></span><span className="font-bold text-emerald-700">Use →</span></button><div className="my-5 flex items-center gap-3"><div className="h-px flex-1 bg-slate-200"/><span className="text-xs font-bold text-slate-400">OR SEARCH A PLACE</span><div className="h-px flex-1 bg-slate-200"/></div><div className="flex gap-2"><input value={addressQuery} onChange={event => setAddressQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void searchAddress(); }} placeholder="Full address, town, village, landmark, PIN..." className="min-w-0 flex-1 rounded-xl border border-slate-200 px-4 py-3.5 outline-none focus:border-amber-400"/><button type="button" onClick={() => void searchAddress()} disabled={addressLoading} className="rounded-xl bg-slate-900 px-5 py-3.5 font-bold text-white disabled:opacity-50">{addressLoading ? 'Searching…' : 'Search'}</button></div>{addressResults.length > 0 && <div className="mt-4 space-y-2"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Choose the exact result</p>{addressResults.map(result => <button type="button" key={`${result.place_id}-${result.lat}`} onClick={() => saveLocation({ latitude: result.lat, longitude: result.lon, label: result.display_name, precision: result.type === 'city' || result.type === 'administrative' ? 'area' : 'exact' })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-left text-sm hover:border-amber-300 hover:bg-amber-50">📍 {result.display_name}</button>)}</div>}<p className="mt-5 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">For the most accurate worker distance, enter the full service address or use GPS at the service address. A broad city selection gives an area-level distance only.</p></div></div>}
