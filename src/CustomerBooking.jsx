...
const SEARCH_ALIASES = [
  [/'cann?aught place/gi, 'Connaught Place'],
  [/'connaught plce/gi, 'Connaught Place'],
  [/'cp delhi/gi, 'Connaught Place, New Delhi'],
  [/'cp new delhi/gi, 'Connaught Place, New Delhi'],
  [/'new delhi/gi, 'New Delhi']
];
...
const normalizePlaceQuery = value => SEARCH_ALIASES.reduce((query, [pattern, replacement]) => query.replace(pattern, replacement), value.trim().replace(/\s+/g, ' '));
...
function CustomerBooking({ onBack, onSelectWorker }) {
  ...
  const searchAddress = async () => {
    const rawQuery = addressQuery.trim();
    if (rawQuery.length < 3) { setError('Enter a town, village, landmark, PIN code, or address.'); return; }
    const query = normalizePlaceQuery(rawQuery);
    setAddressLoading(true); setAddressResults([]); setError('');
    try {
      const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=5&countrycodes=in&accept-language=en&q=${encodeURIComponent(`${query}, India`)}`, { headers: { Accept: 'application/json' } });
      ...
      if (!results.length) setError(`No place found for “${rawQuery}”. Try the area, city, PIN code, or full address.`);
  ...
  const selectLocationResult = result => {
    ...
    if (!Number.isFinite(next.latitude) || !Number.isFinite(next.longitude)) { setError('That location result is invalid. Please choose another.'); return; }
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await apiRequest('/api/bookings', { method: 'POST', token: getStoredToken(), body: { problemDescription: note.trim(), serviceTag: analysis?.service || analysis?.serviceTag || selectedWorker.displayedSkill || selectedWorker.skills?.[0], location: { type: 'Point', coordinates: [serviceLocation.longitude, serviceLocation.latitude] }, workerId: selectedWorker._id } });
  ...
  const precisionMessage = serviceLocation?.precision === 'area'
    ? 'Area selected. Distance is an estimate from this area. Use the full address or GPS for doorstep accuracy.'
