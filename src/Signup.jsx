...
function Signup({ role, onSuccess, onBack, onLogin }) {
  ...
  const [password, setPassword] = useState(''); const [confirmPassword, setConfirmPassword] = useState(''); const [skills, setSkills] = useState([]);
  const [location, setLocation] = useState(''); const [companyName, setCompanyName] = useState(''); const [service, setService] = useState('');
  const [digilockerConnected, setDigilockerConnected] = useState(false); const [error, setError] = useState(''); const [loading, setLoading] = useState(false); const [success, setSuccess] = useState(false);
  const isWorker = role === 'worker'; const isContractor = role === 'contractor'; const isCustomer = role === 'customer';
  const title = useMemo(() => isWorker ? 'Create your worker profile' : isContractor ? 'Create your contractor account' : 'Create your customer account', [isWorker, isContractor]);
  const toggleSkill = (skill) => { setSkills((current) => current.includes(skill) ? current.filter((item) => item !== skill) : [...current, skill]); setError(''); };
  const connectDigiLocker = () => { const url = import.meta.env.VITE_DIGILOCKER_AUTH_URL; if (!url) { setError('DigiLocker access is not configured yet. Add VITE_DIGILOCKER_AUTH_URL after enabling your DigiLocker/API Setu partner credentials.'); return; } window.location.href = url; };
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      const verification = isWorker ? { status: digilockerConnected ? 'verified' : 'pending', provider: digilockerConnected ? 'digilocker' : 'manual' } : undefined;
      const data = await apiRequest(`/api/auth/${role}/register`, { method: 'POST', body: { name: name.trim(), email: email.trim().toLowerCase(), phone: phone.trim(), password, ...(isWorker ? { skills, verification } : {}), ...(isContractor ? { companyName: companyName.trim(), location: location.trim(), service } : {}) } });
      saveAuth(data); setSuccess(true); setTimeout(() => onSuccess(), 900);
    } catch (requestError) { setError(requestError.message || 'Registration failed. Please try again.'); } finally { setLoading(false); }
  };
  ...
  return (
    <main className="min-h-screen bg-[#FFF8F3] px-4 py-6 text-slate-800 sm:px-8">
      ...
      <div className="mx-auto max-w-6xl pt-8"><div className="grid overflow-hidden rounded-[2rem] border border-amber-100 bg-white shadow-2xl lg:grid-cols-[.82fr_1.18fr]">
        <section className="bg-[#FFF1E6] px-7 py-10 sm:px-12 lg:px-14">
          ...
          <div className="mt-8 space-y-4 text-sm font-semibold text-slate-700"><div className="flex gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-sm">✓</span> Simple account creation</div><div className="flex gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-sm">📍</span> Built for local communities</div>{isWorker && <div className="flex gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-sm">🛡️</span> Identity + skill profile</div>}</div>
        </section>
        <section className="px-7 py-10 sm:px-12">{!success ? <form onSubmit={handleSubmit} className="mx-auto max-w-xl">
          ...
          {isWorker && <>
            <div className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5"><div className="flex items-start justify-between gap-4"><div><p className="font-bold text-slate-900">Verify your identity</p><p className="mt-1 text-xs leading-5 text-slate-600">Connect DigiLocker through the approved integration. Anvaya will never pretend a user is verified.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-700">{digilockerConnected ? 'Connected' : 'Not connected'}</span></div><button type="button" onClick={connectDigiLocker} className="mt-4 w-full rounded-xl border border-emerald-200 bg-white px-4 py-3 font-bold text-emerald-800 hover:bg-emerald-50">{digilockerConnected ? '✓ DigiLocker connected' : 'Connect DigiLocker'}</button></div>
